...
class JtrajRightNode : public rclcpp::Node
{
private: 
    ...
    // Joint names for right IIWA arm (7 DOF)
...
public:
    ...
    JtrajRightNode() : Node("jtraj_right_node")
    {
        // Publisher for right arm joint commands
        publisher_ = this->create_publisher<sensor_msgs::msg::JointState>("right_arm/cmd/joint_position", 1);
        
        // Subscription to joint states
        subscription_ = this->create_subscription<sensor_msgs::msg::JointState>(
            "joint_states", 1,
            std::bind(&JtrajRightNode::jointStateCallback, this, std::placeholders::_1));
        
        // Action server for right arm
        action_server_ = rclcpp_action::create_server<Jtraj>(
            this,
            "jtraj_right",
            std::bind(&JtrajRightNode::handle_goal, this, std::placeholders::_1, std::placeholders::_2),
            std::bind(&JtrajRightNode::handle_cancel, this, std::placeholders::_1),
            std::bind(&JtrajRightNode::handle_accepted, this, std::placeholders::_1));
        
        RCLCPP_INFO(this->get_logger(), "Action server 'jtraj_right' started for right IIWA arm.");
    }
    ...
    {
        RCLCPP_INFO(this->get_logger(), "Received goal request for RIGHT arm with tf %f", goal->tf);
        (void)uuid;
    ...
    {
        RCLCPP_INFO(this->get_logger(), "Received request to cancel RIGHT arm goal");
        (void)goal_handle;
    ...
    {
        std::thread{std::bind(&JtrajRightNode::execute, this, std::placeholders::_1), goal_handle}.detach();
    }
    ...
    void execute(const std::shared_ptr<GoalHandleJtraj> goal_handle)
    {
        ...
        while (rclcpp::ok()) {
            ...
            if ((this->now() - start_wait).seconds() > 10.0) {
                RCLCPP_ERROR(this->get_logger(), "[jtraj_right] Timeout: no joint_states received.");
        ...
        // Extract current positions for right arm joints
        std::vector<double> qi = extractRightArmPositions();
    ...
    std::vector<double> extractRightArmPositions()
    {
        std::vector<double> right_positions(JOINT_NAMES.size(), 0.0);
        
        if (!last_joint_state_received_) {
            RCLCPP_WARN(this->get_logger(), "No joint state received, assuming zero positions for right arm.");
            return right_positions;
        }
        
        // Extract positions for right arm joints
        for (size_t i = 0; i < JOINT_NAMES.size(); ++i) {
            ...
            if (it != last_joint_state_.name.end()) {
                ...
                if (index < last_joint_state_.position.size()) {
                    right_positions[i] = last_joint_state_.position[index];
                }
            }
        }
                                   ...
                                   std::shared_ptr<Jtraj::Result> result)
    {
        ...
        for (size_t i = 1; i <= N && rclcpp::ok(); i++) {
            if (goal_handle->is_canceling()) {
                ...
                RCLCPP_INFO(this->get_logger(), "RIGHT arm goal canceled.");
                return;
            }
...
int main(int argc, char **argv)
{
    ...
    auto node = std::make_shared<JtrajRightNode>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}